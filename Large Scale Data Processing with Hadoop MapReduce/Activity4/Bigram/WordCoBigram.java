...
public class WordCoBigram {
	...
	public static class LatinMapper extends Mapper<Object, Text, Text, Text> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			if (value.toString().length() == 0) {
				...
				value2 = value2.replaceAll("[^A-Za-z0-9\\s]", "").trim()
						.replaceAll(" +", " ").replaceAll("j", "i").replaceAll("v", "u");
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			if (hm.containsKey(keySplit[0].toLowerCase()) && hm.containsKey(keySplit[1].toLowerCase())) {
				...
				for (int i = 1; i < listWord1.size(); i++) {
					...
					{
						context.write(new Text(listWord1.get(i)+" "+listWord2.get(j)), new Text(temp + ", Count: " + Integer.toString(count)));
				}
				}
			} 
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "WordCoBigram");
		job.setJarByClass(WordCoBigram.class);
